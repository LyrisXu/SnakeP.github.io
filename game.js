    ...
    move: function() {
		...
		***/
		...
		if ( nextHead[0] == currentCoin[0] && nextHead[1] == currentCoin[1] ){
			...
			releaseBonus(currentCoin[0],currentCoin[1]); //释放像素
			scoreJudge();							

			//如果还有位置可以生产新的食物，表示下一步可以移动
			if (useNextRandomPixelForCoin()){
				if (score!=0 && score%10==0 && useNextRandomPixelForAward()){					
					return GOOD_MOVE;
				} else
					return GOOD_MOVE;
